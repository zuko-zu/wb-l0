...
/* harmony import */ var _cartItem__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! ./cartItem */ "./src/js/modules/cartItem.js");
/* harmony import */ var _missingCartItem__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! ./missingCartItem */ "./src/js/modules/missingCartItem.js");
...
var basket = _data_data__WEBPACK_IMPORTED_MODULE_10__.cartItems || [];
...
var RenderCartItems = function RenderCartItems(data, item, parentSelector) {
  data.forEach(function (_ref) {
    var title = _ref.title,
      ...
      id = _ref.id;
    new item(title, image, alt, color, size, warehouse, seller, price, oldPrice, discount, amount, remaining, id, parentSelector).render();
  });
};
...
var RenderMissingCartItems = function RenderMissingCartItems(data, item, parentSelector) {
...
var calculateProductCount = function calculateProductCount(data) {
  var cartItems = document.querySelectorAll(".cart-item");
  var result = 0;
  cartItems.forEach(function (cartItem, i) {
    var checkbox = cartItem.querySelector(".checkbox__input");
    if (checkbox.checked) {
...
var calculateCartSum = function calculateCartSum(data) {
  var cartItems = document.querySelectorAll(".cart-item");
  var newPriceSum = 0;
  var oldPriceSum = 0;
  cartItems.forEach(function (cartItem, i) {
  ...
  return {
    ...
    oldPriceSum: oldPriceSum
  };
};
var calculateItemSum = function calculateItemSum(data, i) {
  var dataItem = data.filter(function (item) {
    return item.id === i;
  })[0];
  var newPrice = dataItem.price * dataItem.amount;
  var oldPrice = dataItem.oldPrice * dataItem.amount;
  return {
    newPrice: newPrice,
    oldPrice: oldPrice
  };
...
var manageCounter = function manageCounter(productData, incrementButton, decrementButton, countElement) {
  var count = productData.amount;
  ...
  var updateState = function updateState() {
    ...
    incrementButton.disabled = count >= productData.remaining && productData.remaining !== null;
...
var reduceFontSizeOnOverflow = function reduceFontSizeOnOverflow(item) {
  var container = item.querySelector(".cart-item__price");
  var text = item.querySelector(".cart-item__new-price-value");
  var currentFontSize = parseInt(window.getComputedStyle(text).fontSize, 10);
  var margin = 35;
  var containerWidth = container.offsetWidth - margin;
...
var calculateItemsCount = function calculateItemsCount(data) {
  return data.length;
};
...
var calculateItemDiscount = function calculateItemDiscount(data, i) {
  var regularDiscountSum = 0;
  var customerDiscountSum = 0;
  var dataItem = data.filter(function (item) {
    return item.id === i;
  })[0];
  regularDiscountSum = Math.round(dataItem.price * dataItem.discount.discount * dataItem.amount);
  customerDiscountSum = Math.round(dataItem.price * dataItem.discount.customerDiscount * dataItem.amount);
  return {
    regularDiscountSum: regularDiscountSum,
    customerDiscountSum: customerDiscountSum
  };
};
...
var calculateDiscountSum = function calculateDiscountSum(data) {
  var cartItems = document.querySelectorAll(".cart-item");
  var result = 0;
  cartItems.forEach(function (item, i) {
    var checkbox = item.querySelector(".checkbox__input");
    if (checkbox.checked) {
      result += result += Math.round(data[i].oldPrice * (data[i].discount.discount + data[i].discount.customerDiscount) * data[i].amount);
    }
  });
  return result;
};
...
var deleteCartItem = function deleteCartItem(item, id) {
  basket = basket.filter(function (cartItem) {
...
var deleteMissingCartItem = function deleteMissingCartItem(i) {
  var cartItem = document.querySelector(".missing-cart-item[data-id=\"".concat(i, "\"]"));
  cartItem.remove();
};
...
var toggleCartListVisibility = function toggleCartListVisibility(e, i) {
  var cartList = document.querySelectorAll(".cart__item-list");
...
var changeCartListHeader = function changeCartListHeader() {
  var cartList = document.querySelector(".cart__in-stock .cart__item-list");
  var header = document.querySelector(".cart__in-stock .cart__section-header");
  var totalPrice = calculateCartSum(basket).newPriceSum;
  var totalAmount = calculateProductCount(basket);
  var resultString = "".concat(totalAmount, " \u0442\u043E\u0432\u0430\u0440\u043E\u0432 \xB7 ").concat(formatNumber(totalPrice), " \u0441\u043E\u043C");
  if (cartList.classList.contains("open")) {
    var deleteElement = header.querySelector(".cart__checkbox");
    deleteElement.remove();
    header.insertAdjacentHTML("afterbegin", "<p class=\"cart__in-stock-accordion-header\">".concat(resultString, "</p>"));
  } else {
    var _deleteElement = header.querySelector(".cart__in-stock-accordion-header");
    _deleteElement.remove();
...
var toggleButtonText = function toggleButtonText(price) {
  var submitButton = document.querySelector(".order-sum__button");
  var debitingCheckbox = document.querySelector(".order-sum__checkbox input");
  if (debitingCheckbox.checked) {
    submitButton.innerText = "\u041E\u043F\u043B\u0430\u0442\u0438\u0442\u044C ".concat(price, " \u0441\u043E\u043C");
  } else {
...
var updateCountBages = function updateCountBages() {
  countBages.forEach(function (bage) {
...
var updateOrderSumData = function updateOrderSumData(i) {
  var totalNewPrice = document.querySelector(".order-sum__sum-value");
  var totalOldPrice = document.querySelector("#old-sum");
  var discountSum = document.querySelector("#discount-sum");
  var submitButton = document.querySelector(".order-sum__button");
  var totalProductCountElm = document.querySelector(".order-sum__item-label");
  ...
  totalOldPrice.innerText = formatNumber(calculateCartSum(basket).oldPriceSum) + " сом";
  discountSum.innerHTML = "− " + calculateDiscountSum(basket, i) + " сом";
  submitButton.innerText = submitButton.innerText != "Заказать" ? "\u041E\u043F\u043B\u0430\u0442\u0438\u0442\u044C ".concat(formatNumber(calculateCartSum(basket).newPriceSum), " \u0441\u043E\u043C") : submitButton.innerText;
};
...
var updateItemData = function updateItemData(item, i) {
  var price = item.querySelector(".cart-item__new-price-value");
  var oldPrice = item.querySelector(".cart-item__old-price-value");
  var discount = item.querySelector(".cart-item__discount-value");
  var costumerDiscount = item.querySelector(".cart-item__customer-discount-value");
  ...
  discount.innerHTML = "- " + calculateItemDiscount(basket, i).regularDiscountSum + " сом";
  costumerDiscount.innerHTML = "- " + calculateItemDiscount(basket, i).customerDiscountSum + " сом";
};
var updateDeliveryCards = function updateDeliveryCards(data, parent) {
  var template = "\n  <div class=\"delivery__item\">\n    <p class=\"delivery__date delivery__label\">5\u20146 \u0444\u0435\u0432\u0440\u0430\u043B\u044F</p>\n    <div class=\"delivery__imgs\">\n      \n    </div>\n  </div>";
  var deliveryItems = document.querySelectorAll(".delivery__item");
  ...
  var imgContainer = parent.querySelector(".delivery__item:last-child .delivery__imgs");
  data.forEach(function (item) {
    var checkbox = document.querySelector(".cart-item[data-id=\"".concat(item.id, "\"] .checkbox__input"));
    if (checkbox.checked) {
      if (!item.available || item.available <= item.amount) {
        imgContainer.insertAdjacentHTML("beforeend", "\n          <div class=\"delivery__img-wrapper\">\n            <img\n              src=".concat(item.image, "\n              alt=").concat(item.alt, "\n              class=\"delivery__img\"\n            />\n          <span class=\"delivery__item-count count-bage\">").concat(item.available === null ? item.amount : item.available, "</span>\n          </div>\n        "));
      }
      if (item.available && item.available < item.amount) {
        var label = document.querySelector(".delivery__item:last-child .delivery__date");
        label.innerHTML = "7—8 февраля";
        var _ImgsContainer = document.createElement("div");
        _ImgsContainer.classList.add("delivery__imgs");
        parent.insertAdjacentHTML("beforeend", template);
        var _imgContainer = parent.querySelector(".delivery__item:last-child .delivery__imgs");
        _imgContainer.insertAdjacentHTML("beforeend", "\n              <div class=\"delivery__img-wrapper\">\n                <img\n                  src=".concat(item.image, "\n                  alt=").concat(item.alt, "\n                  class=\"delivery__img\"\n                />\n                <span class=\"delivery__item-count count-bage\">").concat(item.amount - item.available, "</span>\n              </div>\n              \n            "));
      }
    }
  });
};
var deliveryInfoContainer = document.querySelector(".delivery__info");
var handleAccordionBtn = function handleAccordionBtn(e, i) {
  if (i === 0) {
    changeCartListHeader(i);
  }
  toggleCartListVisibility(e, i);
};
var cartList = document.querySelector(".cart__item-list");
RenderCartItems(basket, _cartItem__WEBPACK_IMPORTED_MODULE_11__["default"], cartList);
var missingCartList = document.querySelector(".missing-cart__item-list");
RenderMissingCartItems(basket, _missingCartItem__WEBPACK_IMPORTED_MODULE_12__["default"], missingCartList);
updateDeliveryCards(basket, deliveryInfoContainer);
var accordionBtns = document.querySelectorAll(".cart__accordion-btn");
accordionBtns.forEach(function (btn, i) {
  return btn.addEventListener("click", function (e) {
    return handleAccordionBtn(e, i);
  });
});
var debitingCheckbox = document.querySelector(".order-sum__checkbox input");
debitingCheckbox.addEventListener("change", function (e) {
  return toggleButtonText(formatNumber(calculateCartSum(basket).newPriceSum));
});
var deleteMissingBtns = document.querySelectorAll(".missing-cart__item-list .cart-item__btn--delete");
deleteMissingBtns.forEach(function (btn) {
  return btn.addEventListener("click", function (event) {
    deleteMissingCartItem(event.currentTarget.dataset.id);
    var items = document.querySelectorAll(".missing-cart-item");
    var missingCount = document.querySelector(".cart__out-of-stock-count-value");
    missingCount.innerHTML = items.length;
  });
});
var countBages = document.querySelectorAll(".count-bage--total");
var addItemIventListeners = function addItemIventListeners(item, i) {
  var decrementButton = item.querySelector(".cart-item__counter-btn--dec");
  var incrementButton = item.querySelector(".cart-item__counter-btn--inc");
  var deleteButton = item.querySelector(".cart-item__btn--delete");
  var countElm = item.querySelector(".cart-item__count");
  ...
  var eventButtons = item.querySelectorAll(".cart-item__counter-btn");
  var itemCheckbox = item.querySelector(".checkbox__input");
...
var cartItemElements = document.querySelectorAll(".cart__item-list .cart-item");
cartItemElements.forEach(function (item, i) {
  addItemIventListeners(item, i);
});
var selectAllCheckbox = document.querySelector(".cart__checkbox input");
var cartItemCkeckboxes = document.querySelectorAll(".cart-item__checkbox input");
var selectAllCartItems = function selectAllCartItems(e) {
  e.target.checked = true;
  var cartItemCkeckboxes = document.querySelectorAll(".cart-item__checkbox input");
  if (e.target.checked) {
    cartItemCkeckboxes.forEach(function (checkbox) {
      checkbox.checked = true;
    });
  }
};
var toggleSelectAllCheckbox = function toggleSelectAllCheckbox(e) {
  if (!e.target.checked) {
    selectAllCheckbox.checked = false;
  }
};
selectAllCheckbox.addEventListener("change", function (e) {
  return selectAllCartItems(e);
});
cartItemCkeckboxes.forEach(function (checkbox) {
  checkbox.addEventListener("change", function (e) {
    return toggleSelectAllCheckbox(e);
  });
});
...
var CartItem = /*#__PURE__*/function () {
  function CartItem(title, image, alt, color, size, warehouse, seller, price, oldPrice, discount, amount, remaining, id, parentSelector) {
...
submitButton.addEventListener("click", function (e) {
  var inputs = document.querySelectorAll(".recipient__input");
  var labels = document.querySelectorAll(".recipient__label");
  e.preventDefault();
  inputs.forEach(function (input) {
    if (input.value.trim() === "" || !input.validity.valid) {
      input.classList.add("error");
...
var showErrorMessage = function showErrorMessage(input, errorType) {
  ...
  switch (input.id) {
    case "name":
      errorType === "typo" ? idErrorMessage.innerHTML = "Укажите имя" : idErrorMessage.innerHTML = "Укажите имя";
      break;
    case "surname":
      errorType === "typo" ? idErrorMessage.innerHTML = "Введите фамилию" : idErrorMessage.innerHTML = "Введите фамилию";
      break;
    case "mail":
      errorType === "typo" ? idErrorMessage.innerHTML = "Проверьте адрес электронной почты" : idErrorMessage.innerHTML = "Укажите электронную почту";
      break;
    case "phone":
      errorType === "typo" ? idErrorMessage.innerHTML = "Формат: +9 999 999 99 99" : idErrorMessage.innerHTML = "Укажите номер телефона";
      break;
    case "identification-number":
      errorType === "typo" ? idErrorMessage.innerHTML = "Проверьте ИНН" : idErrorMessage.innerHTML = "Укажите ИНН";
...
/***/ "./src/js/modules/missingCartItem.js":
/*!*******************************************!*\
  !*** ./src/js/modules/missingCartItem.js ***!
  \*******************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
function _toPrimitive(input, hint) { if (_typeof(input) !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (_typeof(res) !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); }
var CartItem = /*#__PURE__*/function () {
  function CartItem(title, image, alt, color, size, id, parentSelector) {
    _classCallCheck(this, CartItem);
    this.title = title;
    this.image = image;
    this.alt = alt;
  ...
  _createClass(CartItem, [{
    key: "render",
    value: function render() {
      this.parentSelector.innerHTML += "\n      <li class=\"cart__missing-item missing-cart-item\" data-id=".concat(this.id, ">\n        <div class=\"missing-cart-item__photo-wrapper\">\n          <img\n            class=\"missing-cart-item__photo\"\n            src=").concat(this.image, "\n            alt=").concat(this.alt, "\n            width=\"80\"\n            height=\"106\"\n          > \n        </div>\n        <div class=\"missing-cart-item__info\">\n          <h3 class=\"missing-cart-item__title\" title=\"").concat(this.title, "\">\n            <a class=\"missing-cart-item__link\" href=\"#\">").concat(this.title, "</a>\n          </h3>\n          ").concat(this.color ? "<p class=\"missing-cart-item__color\">\u0426\u0432\u0435\u0442:  ".concat(this.color, "</p>") : "", "\n          ").concat(this.size ? "<p class=\"missing-cart-item__size\">\u0420\u0430\u0437\u043C\u0435\u0440:  ".concat(this.size, "</p>") : "", "  \n        </div>\n        \n        <div class=\"missing-cart-item__btns\">\n          <button type=\"button\" class=\"cart-item__btn cart-item__btn--favorites\">\n            <span class=\"visually-hidden\"\n              >\u0414\u043E\u0431\u0430\u0432\u0438\u0442\u044C \u0432 \u0441\u043F\u0438\u0441\u043E\u043A \u0438\u0437\u0431\u0440\u0430\u043D\u043D\u044B\u0445 \u0442\u043E\u0432\u0430\u0440\u043E\u0432</span\n            >\n            <svg\n              focusable=\"false\"\n              aria-hidden=\"true\"\n              xmlns=\"http://www.w3.org/2000/svg\"\n              width=\"20\"\n              height=\"20\"\n              viewBox=\"0 0 20 20\"\n              fill=\"none\"\n            >\n              <path\n                fill-rule=\"evenodd\"\n                clip-rule=\"evenodd\"\n                d=\"M3.03386 4.05857C2.2658 4.75224 1.76675 5.83284 1.99484 7.42928C2.22323 9.02783 3.26485 10.6852 4.80427 12.3478C6.25856 13.9184 8.10953 15.4437 9.99986 16.874C11.8902 15.4437 13.7412 13.9184 15.1955 12.3478C16.7349 10.6852 17.7765 9.02783 18.0049 7.4293C18.2329 5.83285 17.7339 4.75224 16.9658 4.05856C16.1765 3.34572 15.0549 3 13.9999 3C12.1318 3 11.0922 4.08479 10.5176 4.68443C10.458 4.7466 10.4034 4.80356 10.3534 4.85355C10.1582 5.04882 9.84157 5.04882 9.64631 4.85355C9.59632 4.80356 9.54173 4.7466 9.48215 4.68443C8.90748 4.08479 7.86788 3 5.99986 3C4.94486 3 3.82316 3.34573 3.03386 4.05857ZM2.36361 3.31643C3.3736 2.40427 4.75192 2 5.99986 2C8.07114 2 9.3453 3.11257 9.99986 3.77862C10.6544 3.11257 11.9286 2 13.9999 2C15.2478 2 16.6261 2.40428 17.6361 3.31644C18.6673 4.24776 19.2668 5.66715 18.9948 7.5707C18.7232 9.47217 17.5148 11.3148 15.9293 13.0272C14.3354 14.7486 12.3063 16.3952 10.2999 17.9C10.1221 18.0333 9.87764 18.0333 9.69986 17.9C7.69344 16.3952 5.66434 14.7485 4.0705 13.0272C2.48494 11.3148 1.27656 9.47217 1.00489 7.57072C0.732921 5.66716 1.3324 4.24776 2.36361 3.31643Z\"\n                fill=\"black\"\n              />\n            </svg>\n          </button>\n          <button type=\"button\" class=\"cart-item__btn cart-item__btn--delete\" data-id=").concat(this.id, ">\n            <span class=\"visually-hidden\"\n              >\u0423\u0434\u0430\u043B\u0438\u0442\u044C \u0438\u0437 \u043A\u043E\u0440\u0437\u0438\u043D\u044B</span\n            >\n            <svg\n              focusable=\"false\"\n              aria-hidden=\"true\"\n              xmlns=\"http://www.w3.org/2000/svg\"\n              width=\"20\"\n              height=\"20\"\n              viewBox=\"0 0 20 21\"\n              fill=\"none\"\n            >\n              <path\n                fill-rule=\"evenodd\"\n                clip-rule=\"evenodd\"\n                d=\"M2.5 5.66663C2.5 5.39048 2.72386 5.16663 3 5.16663H17C17.2761 5.16663 17.5 5.39048 17.5 5.66663C17.5 5.94277 17.2761 6.16663 17 6.16663H3C2.72386 6.16663 2.5 5.94277 2.5 5.66663Z\"\n                fill=\"black\"\n              />\n              <path\n                fill-rule=\"evenodd\"\n                clip-rule=\"evenodd\"\n                d=\"M3.4584 5.16663H16.5059L15.6411 16.3592C15.5405 17.6613 14.4546 18.6666 13.1486 18.6666H6.84639C5.54299 18.6666 4.45829 17.6652 4.35435 16.366L3.4584 5.16663ZM4.5416 6.16663L5.35117 16.2862C5.41353 17.0658 6.06435 17.6666 6.84639 17.6666H13.1486C13.9322 17.6666 14.5837 17.0635 14.6441 16.2822L15.4256 6.16663H4.5416Z\"\n                fill=\"black\"\n              />\n              <path\n                fill-rule=\"evenodd\"\n                clip-rule=\"evenodd\"\n                d=\"M13 6.16663H7V4.13538C7 3.32421 7.65758 2.66663 8.46875 2.66663H11.5312C12.3424 2.66663 13 3.32421 13 4.13538V6.16663ZM8.46875 3.66663C8.20987 3.66663 8 3.87649 8 4.13538V5.16663H12V4.13538C12 3.87649 11.7901 3.66663 11.5312 3.66663H8.46875Z\"\n                fill=\"black\"\n              />\n            </svg>\n          </button>\n        </div>\n      </li>\n    ");
    }
  }]);
  return CartItem;
}();
...
var addressModalOpenBtn = document.querySelector("#addressOpenBtn");
var deliveryModalOpenBtn = document.querySelector(".order-form__edit-btn");
addressModalOpenBtn.addEventListener("click", function () {
  openModal(".address-modal");
});
deliveryModalOpenBtn.addEventListener("click", function () {
  openModal(".address-modal");
});
...
var selectDeliveryTypeBtns = document.querySelectorAll(".address-modal__option-button");
var pickUAddressesList = document.querySelector(".address-modal__list-pick-up");
var addressesList = document.querySelector(".address-modal__list-courier");
selectDeliveryTypeBtns.forEach(function (btn) {
  btn.addEventListener("click", function (e) {
    selectDeliveryTypeBtns.forEach(function (btn) {
      btn.classList.remove("active");
    });
    e.target.classList.add("active");
    if (e.target.classList.contains("active") && e.target.classList.contains("address-type-pick-up")) {
      addressesList.style.display = "none";
      pickUAddressesList.style.display = "block";
    }
    if (e.target.classList.contains("active") && e.target.classList.contains("address-type-courier")) {
      addressesList.style.display = "block";
      pickUAddressesList.style.display = "none";
    }
  });
});
var addresses = addressModal.querySelectorAll("li");
var selectBtn = document.querySelector(".address-modal__select-btn");
addresses.forEach(function (address) {
  var deleteBtn = address.querySelector(".modal__delete-btn");
  deleteBtn.addEventListener("click", function () {
    address.remove();
  });
  var radioBtn = address.querySelector("input[type='radio']");
  var labelText = address.querySelector("label").innerText;
...
var updateAddressData = function updateAddressData(address) {
  var selectDeliveryTypeBtns = document.querySelectorAll(".address-modal__option-button");
  var orderTitle = document.querySelector(".address-subtitle");
  var orderAddress = document.querySelector(".order-sum__delivery-address");
  var deliveryTitle = document.querySelector(".delivery__address-label");
  var deliveryAddress = document.querySelector(".delivery__address-value");
  var deliveryRating = document.querySelector(".delivery__address-rating");
  var deliverySchedule = document.querySelector(".delivery__address-schedule");
  var deliveryRatingIcon = document.querySelector(".delivery__address-rating-icon");
  selectDeliveryTypeBtns.forEach(function (btn) {
    if (btn.classList.contains("address-type-courier") && btn.classList.contains("active")) {
    ...
    if (btn.classList.contains("address-type-pick-up") && btn.classList.contains("active")) {
...
var paymentModalOpenBtn = document.querySelector(".payment .order-form__edit-btn");
var paymentModalOrderOpenBtn = document.querySelector("#paymentOpenBtn");
paymentModalOpenBtn.addEventListener("click", function () {
  openModal(".payment-modal");
});
paymentModalOrderOpenBtn.addEventListener("click", function () {
  openModal(".payment-modal");
});
var paymentCardImg = document.querySelector(".payment .payment__card-type");
var paymentOrderCardImg = document.querySelector(".order-sum .payment__card-type");
var paymentModalList = document.querySelectorAll(".payment-modal__list li");
var paymentSelectBtn = document.querySelector(".payment-modal__select-btn");
paymentSelectBtn.addEventListener("click", function (e) {
  paymentModalList.forEach(function (item) {
...
paymentModalList.forEach(function (item) {});
var getSelectedPaymentItem = function getSelectedPaymentItem() {
  _toConsumableArray(paymentModalList).forEach(function (item) {
    if (item.checked) {}
  });
};
